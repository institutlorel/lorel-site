...
export function ContactClient({ waSettings }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (form.formation) {
      ...
      if (formation) fullMessage += ` — Intéressé(e) par : ${formation.titreFr}`;
    }
          ...
          <div className="lg:grid lg:grid-cols-12 gap-10">
              ...
              <div className="bg-white rounded-2xl p-6 shadow-card">
                ...
                {submitted ? (
                  ...
                  <form onSubmit={handleSubmit} noValidate>
                    <input name="website" value={honeypot} onChange={(e) => setHoneypot(e.target.value)} tabIndex={-1} autoComplete="off" aria-hidden="true" style={{ position: "absolute", left: "-9999px", opacity: 0, pointerEvents: "none" }} />
                    ...
                    {submitError && <p className="font-body text-sm text-red-600 mb-4">{submitError}</p>}

                    <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 bg-brand-blue hover:bg-brand-blue-light disabled:opacity-60 disabled:cursor-not-allowed text-white font-body font-bold text-sm py-3.5 rounded-sm transition-colors">
                      {loading ? (
                        <>
            ...
            <div className="lg:col-span-5 mt-8 lg:mt-0 space-y-4">
              <div className="bg-white rounded-xl p-5 shadow-card flex items-start gap-4">
                ...
                <div>
                  <p className="font-body font-semibold text-text-primary text-sm">contact@institutlorel.com</p>
                </div>
              </div>
