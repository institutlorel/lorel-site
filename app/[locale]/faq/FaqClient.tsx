...
export function FaqClient({ waNumber }: Props) {
    ...
    <>
      ...
      <div className="bg-brand-dark relative overflow-hidden">
        ...
        <Container className="relative z-10 py-16 lg:py-20">
          ...
          <h1 className="font-display font-bold text-white mb-4" style={{ fontSize: "clamp(2.2rem, 5vw, 4rem)" }}>Questions fréquentes</h1>
          <p className="font-body text-white/60 text-base max-w-md">Tout ce que vous voulez savoir sur nos formations, certifications et services.</p>
        </Container>
      </div>
      ...
      <section className="bg-brand-cream py-12 lg:py-16">
        <Container>
          <div className="max-w-lg mx-auto mb-8">
            <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Rechercher une question..." className="w-full font-body text-sm border border-gray-200 rounded-sm px-4 py-3 bg-white focus:outline-none focus:border-brand-blue transition-colors placeholder:text-text-muted" />
          </div>
          ...
          <div className="max-w-2xl mx-auto space-y-8">
            {filtered.length === 0 || filtered.every((c) => c.questions.length === 0) ? (
              <p className="font-body text-sm text-text-muted text-center py-8">Aucune question trouvée pour cette recherche.</p>
            ) : (
              filtered.map((cat) =>
      ...
      <section className="bg-white py-12 border-t border-gray-100">
        <Container className="text-center">
          <p className="font-display font-bold text-brand-dark text-xl mb-2">Vous ne trouvez pas votre réponse ?</p>
          <p className="font-body text-sm text-text-secondary mb-6">Notre équipe est disponible pour répondre à toutes vos questions personnalisées.</p>
